...
<script>
    $(function () {
        $("#refrigeration").slider({ max: 27, min: 12, step: 1, change: function (e, ui) { $('#txtValue_refrigeration').val(ui.value); }, value: parseInt($('#txtValue_refrigeration').val(),10)||0/*初始化值，不设置默认为0*/ });
        var v = $("#refrigeration").slider('value'), opt = $("#refrigeration").slider('option');
        if(v == "12"){
        	$( "#amount_refrigeration" ).html("还没调温度");
        }
        ...
        $("#refrigeration_1").click(function () {
            ...
            $( "#amount_refrigeration" ).html(temperature);
        ...
        $("#heating").slider({ max: 42, min: 27, step: 1, change: function (e, ui) { $('#txtValue_heating').val(ui.value); }, value: parseInt($('#txtValue_refrigeration').val(),10)||0/*初始化值，不设置默认为0*/ });
        var v = $("#heating").slider('value'), opt = $("#heating").slider('option');
        if(v == "27"){
        	$( "#amount_heating" ).html("还没调温度");
        }
        ...
        $("#heating_1").click(function () {
            ...
            $( "#amount_heating" ).html(temperature);
    ...
    function refrigeration_study(){
    	if($("#txtValue_refrigeration").val()==12){
    		alert("还没调温度,请先调下温度在控制");
    	}else{
    		window.location.href='<%=basePath%>hostdevice/air_study.do?VALUES='+$("#txtValue_refrigeration").val()+'&GUEST_ROOM='+'${pd.GUEST_ROOM}'+'&DEVICE_CODE='+'${pd.DEVICE_CODE}'+'&DEVICE_ADDRESS='+'${pd.DEVICE_ADDRESS}';
    	}
    }
    
    
    function heating_study(){
    	if($("#txtValue_heating").val()==27){
    		alert("还没调温度,请先调下温度在控制");
    	}else{
    		window.location.href='<%=basePath%>hostdevice/air_study.do?VALUES='+$("#txtValue_heating").val()+'&GUEST_ROOM='+'${pd.GUEST_ROOM}'+'&DEVICE_CODE='+'${pd.DEVICE_CODE}'+'&DEVICE_ADDRESS='+'${pd.DEVICE_ADDRESS}';
    	}
    }
</script>
<style>
...
#amount_refrigeration{margin:0px 10px;margin-top: 60px; display:block; text-align: center; font-size:20px;color:white;font-family: 楷体,SimKai;}
...
#amount_heating{margin:0px 10px;display:block; text-align: center; font-size:20px;color:white;font-family: 楷体,SimKai;}
...
<body style="background-color: gray;">
	...
	<div id="zhong">
		<div>
			<a href="<%=basePath%>hostdevice/air_study.do?DEVICE_ADDRESS=${pd.DEVICE_ADDRESS}&GUEST_ROOM=${pd.GUEST_ROOM}&DEVICE_CODE=${pd.DEVICE_CODE}&VALUES=11">
				<img src="uploadFiles/close_pressed.png" style="height: 60px; width: 60px;" />
			</a> 
			<a href="<%=basePath%>hostdevice/air_study.do?DEVICE_ADDRESS=${pd.DEVICE_ADDRESS}&GUEST_ROOM=${pd.GUEST_ROOM}&DEVICE_CODE=${pd.DEVICE_CODE}&VALUES=12">
		...
		<div>
			 ...
			 <div>
			 	<span id="amount_refrigeration" onclick="refrigeration_study()"></span>	
			 <br>
			 	<input type="button" value="+" id="refrigeration_1"/>制冷<input type="button" value="-" id="refrigeration_2" />
			 </div>
			<br /><br>
			<input id="txtValue_refrigeration" type="hidden" value="12" readonly="readonly" />
	...
	<div id="zhong1">
		 <span></span>
			 <div id="heating"></div>
			 <div>
			 	<span id="amount_heating" onclick="heating_study()"></span>	
			 <br>
			 	<input type="button" value="+" id="heating_1"/>制热<input type="button" value="-" id="heating_2" />
			 </div>
			<br /><br>
			<input id="txtValue_heating" type="hidden" value="27" readonly="readonly" />
	</div>
	<div id="zhongxin" ></div>
	<!-- 引入 -->
