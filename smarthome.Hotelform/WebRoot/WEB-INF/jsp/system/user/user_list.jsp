...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center"><i class="icon-time hidden-phone"></i>最近登录</th>
					...
					<c:when test="${not empty userList}">
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<%-- <td>${user.NUMBER }</td> --%>
								<td style="vertical-align:middle;text-align: center;"><a>${user.USERNAME }</a></td>
								<%-- <td>${user.NAME }</td> --%>
								...
								<td style="vertical-align:middle;text-align: center;">${user.LAST_LOGIN}</td>
								<td style="vertical-align:middle;text-align: center;">${user.IP}</td>
								<td style="vertical-align:middle;text-align: center;">
									<img  src="<%=basePath%>${user.QRCODE}"style="width: 100px;height: 100px" onclick="amplification('${user.QRCODE}');">
								</td>
								<td >
									<%-- <div class='hidden-phone visible-desktop btn-group'>
										
										<c:if test="${QX.FW_QX == 1 }">
										<a class='btn btn-mini btn-warning' title="发送短信" onclick="sendSms('${user.PHONE }','${user.USERNAME }','${user.PASSWORD }');"><i class='icon-envelope'></i></a>
										</c:if>
									...
									<c:if test="${QX.edit == 1 }">
											<a class='btn btn-mini btn-info' title="编辑" onclick="editUser('${user.USER_ID }');" style="float: left;"><i class='icon-edit'></i></a>
											<%-- <c:if test="${user.USERNAME != 'admin'}"></c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		<script type="text/javascript" src="js/jz.js"></script><!--提示框-->
		...
		//去发送短信页面
		function sendSms(phone,username,pwd){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="发送短信";
			 diag.URL = '<%=basePath%>head/goSendSms.do?PHONE='+phone+'&username='+username+'&pwd='+pwd+'&msg=appuser';
			 diag.Width = 600;
			 diag.Height = 265;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		//二维码放大
		function amplification(IMG){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="二维码放大";
			 diag.URL = '<%=basePath%>user/goAmplification.do?IMG='+IMG;
			 diag.Width = 1400;
			 diag.Height = 800;
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>user/goEditUs.do?USER_ID='+user_id;
			 diag.Width = 225;
			 diag.Height = 415;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId,msg){
			if(userId == 'e9aa1dbbe1a74c68b56bff27c3ba715c'){
				alert("改账号不能删除");
			}else{
