...
<body>
	<form action="modelinfo/${msg }.do" name="Form" id="Form" method="post">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<td>
					<c:choose>
						<c:when test="${not empty model}">
							<select name="MODEL_ID" class="form-control checkacc" id="MODEL_ID">
								<c:forEach items="${model}" var="var" varStatus="vs">
									<option value="${var.MODEL_ID}">${var.NAME}</option>			
								</c:forEach>
							</select>
			...
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">设备地址码:</td>
				<td>
					<%-- <input type="text" name="DEVICE_ADDRESS" id="DEVICE_ADDRESS" value="${pd.DEVICE_ADDRESS}" maxlength="32" placeholder="这里输入DEVICE_ADDRESS" title="DEVICE_ADDRESS"/> --%>
					<c:choose>
						<c:when test="${not empty getAll}">
							<select name="DEVICE_ADDRESS" class="form-control checkacc" id="DEVICE_ADDRESS">
								<c:forEach items="${getAll}" var="var" varStatus="vs">
									<option value="${var.DEVICE_ADDRESS}">${var.NICK_NAME}</option>			
								</c:forEach>
							</select>
			...
			<tr>
				...
				<td>
					<%-- <input type="text" name="CONTROL_COMMAND" id="CONTROL_COMMAND" value="${pd.CONTROL_COMMAND}" maxlength="32" placeholder="这里输入CONTROL_COMMAND" title="CONTROL_COMMAND"/> --%>
			...
			<tr>
				...
				<td>
					<%-- <input type="text" name="DELAY_VALUES" id="DELAY_VALUES" value="${pd.DELAY_VALUES}" maxlength="32" placeholder="这里输入DELAY_VALUES" title="DELAY_VALUES"/> --%>
